...
class VideoMediaScannerInstrumentedTest {
    @Test
    fun scanner_readsVideoFromDeviceMediaStore() {
        val context =
            InstrumentationRegistry
                .getInstrumentation()
                .targetContext

        val result = VideoMediaScanner(context).scan()

        assertTrue(
            "Expected MediaStore scan to succeed, but got $result",
            result is VideoMediaScanResult.Success,
        )

        val items =
            (result as VideoMediaScanResult.Success).items
